...
namespace MyBIIntegrationSBAPI.Integration {
	internal class InitialDataRetrieval {
		public static void ExportCustomers(Screen screen) {
			Console.WriteLine("Retrieving the list of customers with contracts...");

			//Get the schema of the Customers (AR303000) form and
			//configure the sequence of commands
			AR303000Content custSchema = PX.Soap.Helper.GetSchema<AR303000Content>(screen);
			...
			string[][] customerData = screen.AR303000Export(commands, null, 0, true, false);
