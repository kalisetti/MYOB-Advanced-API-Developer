...
namespace MyStoreIntegrationSBAPI.Integration {
	internal class Reports {
		...
		//on the Invoice & Memo (SO643000) report form
		public static void GetPrintableInvoice(Screen context) {
			//Invoice data
			string docType = "Invoice";
			string invoiceNbr = "INV000045";

			//Get the schema of the Invoice & Memo form (SO643000)
			//by using the screen-based API wrapper
			Content invoiceFormSchema = PX.Soap.Helper.GetSchema<Content>(context);
